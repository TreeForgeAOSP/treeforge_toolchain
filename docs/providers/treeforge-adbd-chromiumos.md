# TreeForge ChromiumOS Early-Userspace ADB Daemon

## Overview

`adbd.treeforge_chromiumos` is TreeForge's ARM64 device-side ADB daemon
for ChromiumOS and minimal early-userspace bring-up environments.
...
- Module: `adbd.treeforge_chromiumos`
- Binary: `treeforge-adbd-chromiumos`
- Architecture: ARM64
...
- Compile profile: `TREEFORGE_CHROMIUMOS_EARLY_USERSPACE=1`

## Purpose

The daemon provides early root ADB access during ChromiumOS bring-up.

It is intended to work before a complete ChromiumOS userspace exists and
...
Initial ChromiumOS bring-up uses:
...
Those belong to TreeForge ChromiumOS PID1.

## Android USB Properties

The ChromiumOS profile does not depend on:
...
The ChromiumOS initramfs will provide:
...
The ChromiumOS profile supplies:

- `HOME=/`
- `HOSTNAME=treeforge-chromiumos`
- `LOGNAME=root`
- `USER=root`
- `SHELL=/system/bin/sh`
- `TMPDIR=/tmp`
...
ChromiumOS early userspace does not require Android's Recovery
...
required for the supported ChromiumOS startup path.
...
Those services are not considered supported ChromiumOS features unless
...
active-connection counter in ChromiumOS mode.
...
For the ChromiumOS early-userspace profile, sync bypasses Android-specific
filesystem policy:
...
The initial ChromiumOS contract does not include:
...
Expected initial layout:
    ...
    /system/bin/treeforge-adbd-chromiumos
    /system/bin/linker64
    /system/bin/sh -> /bin/sh
    /system/lib64/...
    /bin/sh
...
Expected TreeForge PID1 sequence:
    ...
    launch treeforge-adbd-chromiumos
...
Required before packaging:
    ...
    [PASS] adbd.treeforge_chromiumos builds
...
`tools/adb/patches/treeforge-chromiumos-early-userspace-adbd.patch`

The ChromiumOS profile must remain independent from:
